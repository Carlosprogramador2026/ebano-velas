    ...
    <section id="repurpose-section" className="relative w-full bg-[#0a0908] py-14 sm:py-20 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center max-w-2xl mx-auto mb-10 sm:mb-14">
          ...
          <h2
            className="font-cinzel font-bold tracking-[0.16em] text-[#f2ebe1] uppercase leading-tight"
        ...
        {/* 2-Column Responsive Layout: Left Active Stage / Right 5 Items List (vertical 9:16 reference on mobile) */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-center">
          {/* Main Stage / Featured Highlight (Left 7 cols on desktop, top on mobile) */}
          <div className="lg:col-span-7 flex flex-col items-center w-full">
            <div className="relative w-full aspect-[4/3] sm:aspect-square max-w-md mx-auto rounded-2xl overflow-hidden border border-[#c6a779]/30 bg-[#14120e] shadow-[0_16px_40px_rgba(0,0,0,0.7)] group">
              <img
              ...
              {/* Card Label Tag */}
              <div className="absolute top-3.5 left-3.5 bg-black/65 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-[#c6a779]/40 flex items-center gap-2">
              ...
              <div className="absolute bottom-0 inset-x-0 p-4 sm:p-6 text-left">
                <p className="text-xs sm:text-base text-[#ede4d8] font-medium leading-relaxed drop-shadow">
                  {currentItem.desc[language]}
                </p>
                <div className="flex items-center gap-2 mt-1.5 pt-1.5 border-t border-white/10 text-xs text-[#c6a779]">
                  <Info className="w-3.5 h-3.5 flex-shrink-0 text-[#d4af37]" />
                  <span className="font-light italic line-clamp-2">{currentItem.tip[language]}</span>
                </div>
              </div>
            </div>

            {/* Quick action buttons below image with 44px min tap target */}
            <div className="flex items-center gap-2 mt-3.5 w-full max-w-md justify-between px-1">
              <button
                onClick={() => setShowHowToClean(!showHowToClean)}
                className="min-h-[44px] text-xs font-cinzel tracking-wider text-[#c6a779] hover:text-[#f2ebe1] flex items-center gap-1.5 transition-colors py-2 px-2 cursor-pointer"
              >
                <span>{language === 'en' ? 'How to clean vessel' : 'Como limpar a peça'}</span>
                <ArrowUpRight className="w-3.5 h-3.5" />
              </button>

              <button
                onClick={onOpenQuickBuy}
                className="min-h-[44px] text-xs font-cinzel font-semibold tracking-wider text-[#0a0908] bg-[#c6a779] hover:bg-[#d4af37] px-4 py-2 rounded-full transition-all cursor-pointer flex items-center justify-center"
              >
