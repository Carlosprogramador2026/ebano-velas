    ...
    <section className="relative w-full bg-[#090807] py-14 sm:py-20 border-t border-[#c6a779]/20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center max-w-xl mx-auto mb-8 sm:mb-12">
          ...
          {/* Variant toggle (Tall Fluted Column vs Shallow Scalloped Dish) */}
          <div className="flex items-center justify-center gap-2 mt-4 p-1 rounded-full bg-[#14120e] border border-[#c6a779]/30 w-fit mx-auto">
            <button
              onClick={() => setActivePosterTab('tall')}
              className={`px-4 py-1.5 rounded-full text-xs font-cinzel font-semibold tracking-wider transition-all ${
                activePosterTab === 'tall'
                  ? 'bg-[#c6a779] text-black shadow-md'
                  : 'text-[#ede4d8]/70 hover:text-white'
              }`}
            >
              {language === 'en' ? 'Tall Fluted Column' : 'Coluna Canelada Alta'}
            </button>
            <button
            ...
            >
              {language === 'en' ? 'Shallow Tart Bowl' : 'Prato Canelado Baixo'}
            </button>
          </div>
        </div>

        {/* The Poster Frame (Pixel-perfect reproduction of attached artwork) */}
        <div className="relative mx-auto max-w-4xl bg-[#0d0c0a] border border-[#c6a779]/40 rounded-2xl sm:rounded-3xl p-5 sm:p-8 lg:p-10 shadow-[0_25px_60px_rgba(0,0,0,0.85)] overflow-hidden">
          {/* Subtle gold inner border */}
          <div className="pointer-events-none absolute inset-2.5 sm:inset-3 border border-[#c6a779]/15 rounded-xl sm:rounded-2xl" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-8 relative z-10">
            ...
            <div className="md:col-span-8 flex flex-col justify-between">
              ...
              <div className="text-left mb-4 sm:mb-6">
                ...
                <h3 className="font-cinzel text-3xl sm:text-5xl font-black tracking-[0.14em] text-[#f5ebd8] uppercase leading-[1.05]">
                  {language === 'en' ? (
                    <>
              ...
              <div className="relative aspect-[4/3] sm:aspect-[16/11] rounded-xl overflow-hidden border border-[#c6a779]/25 bg-[#14120e] shadow-xl group">
                <img
                  ...
                  style={{
                    objectPosition: activePosterTab === 'tall' ? 'center 40%' : 'center center',
                  }}
                />
                ...
                <div className="absolute bottom-3 inset-x-3 flex items-center justify-between sm:justify-start sm:gap-6 bg-black/60 backdrop-blur-md px-3 sm:px-4 py-2 rounded-xl border border-[#c6a779]/30">
                  {/* Badge 1 */}
                  <div className="flex items-center gap-2">
                    <LeafCircleIcon className="w-6 h-6 text-[#d4af37] flex-shrink-0" />
                    <div>
                      <span className="font-cinzel text-[9px] sm:text-[10px] font-bold text-[#f2ebe1] tracking-wider block uppercase">
                        {language === 'en' ? 'PLANT-BASED WAX' : 'CERA VEGETAL'}
                      </span>
                      <span className="text-[8px] sm:text-[9px] text-[#c6a779] block">
                        {language === 'en' ? 'clean & sustainable burn' : 'queima limpa e sustentável'}
                      </span>
                    </div>
                  </div>

                  <div className="hidden sm:block w-[1px] h-7 bg-[#c6a779]/30" />

                  {/* Badge 2 */}
                  <div className="flex items-center gap-2">
                    <ClockCircleIcon className="w-6 h-6 text-[#d4af37] flex-shrink-0" />
                    <div>
                      <span className="font-cinzel text-[9px] sm:text-[10px] font-bold text-[#f2ebe1] tracking-wider block uppercase">
                        {language === 'en' ? 'LONG-LASTING' : 'LONGA DURAÇÃO'}
                      </span>
                      <span className="text-[8px] sm:text-[9px] text-[#c6a779] block">
                        {language === 'en' ? 'moments that endure' : 'momentos que perduram'}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {/* Bottom 4 Feature Pillars (Exact match to reference poster footer) */}
              <div className="grid grid-cols-4 gap-2 pt-6 mt-4 border-t border-[#c6a779]/20 divide-x divide-[#c6a779]/15">
                {/* 1. Fragrance */}
                <div className="flex flex-col items-center text-center px-1">
                  <LotusIcon className="w-5 h-5 sm:w-6 sm:h-6 text-[#d4af37] mb-1.5" />
                  <span className="font-cinzel text-[8px] sm:text-[10px] font-bold text-[#ede4d8] uppercase tracking-wider leading-tight">
                  ...
                  <span className="font-cormorant italic text-[8px] sm:text-[10px] text-[#c6a779] mt-0.5">
                    {language === 'en' ? 'perfumes and welcomes' : 'perfuma e acolhe'}
                  </span>
                </div>

                {/* 2. Minimalist */}
                <div className="flex flex-col items-center text-center px-1 pl-2">
                  <DiamondIcon className="w-5 h-5 sm:w-6 sm:h-6 text-[#d4af37] mb-1.5" />
                  <span className="font-cinzel text-[8px] sm:text-[10px] font-bold text-[#ede4d8] uppercase tracking-wider leading-tight">
                    {language === 'en' ? 'MINIMALIST DESIGN' : 'DESIGN MINIMALISTA'}
                  </span>
                  <span className="font-cormorant italic text-[8px] sm:text-[10px] text-[#c6a779] mt-0.5">
                    {language === 'en' ? 'elegance in every detail' : 'elegância em cada detalhe'}
                  </span>
                </div>

                {/* 3. Gift */}
                <div className="flex flex-col items-center text-center px-1 pl-2">
                  <GiftIcon className="w-5 h-5 sm:w-6 sm:h-6 text-[#d4af37] mb-1.5" />
                  <span className="font-cinzel text-[8px] sm:text-[10px] font-bold text-[#ede4d8] uppercase tracking-wider leading-tight">
                    {language === 'en' ? 'PERFECT GIFT' : 'PRESENTE PERFEITO'}
                  </span>
                  <span className="font-cormorant italic text-[8px] sm:text-[10px] text-[#c6a779] mt-0.5">
                    {language === 'en' ? 'for all occasions' : 'para todas ocasiões'}
                  </span>
                </div>

                {/* 4. Handmade */}
                <div className="flex flex-col items-center text-center px-1 pl-2">
                  <HandmadeHeartIcon className="w-5 h-5 sm:w-6 sm:h-6 text-[#d4af37] mb-1.5" />
                  <span className="font-cinzel text-[8px] sm:text-[10px] font-bold text-[#ede4d8] uppercase tracking-wider leading-tight">
                    {language === 'en' ? 'HANDMADE' : 'FEITO À MÃO'}
                  </span>
                  <span className="font-cormorant italic text-[8px] sm:text-[10px] text-[#c6a779] mt-0.5">
                    {language === 'en' ? 'with care and purpose' : 'com carinho e propósito'}
                  </span>
                </div>
              </div>
            </div>
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        <img
                          ...
                          className="w-11 h-11 sm:w-12 sm:h-12 rounded-lg object-cover border border-white/10 flex-shrink-0"
                        />
                        <div>
                          <div className="flex items-center gap-1.5">
                            {getRepurposeIcon(idx)}
                            <span className="font-cinzel text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-[#ede4d8]">
                              {item.name[language]}
                            </span>
                          </div>
        ...
        <div className="mt-8 flex justify-center">
          <button
            ...
            className="min-h-[48px] px-8 rounded-full bg-gradient-to-r from-[#d4af37] via-[#c6a779] to-[#b38e4a] text-black font-cinzel text-xs font-bold tracking-[0.16em] uppercase flex items-center justify-center gap-2 shadow-lg hover:brightness-110 active:scale-95 transition-all"
          >
